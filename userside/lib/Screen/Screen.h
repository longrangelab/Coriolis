...
public:
  ...
  virtual void drawInterface(int batteryPercent, int signalStrength, bool hitFlag, int distance,  const char* targetAddress, int windDirection, int windSpeed,float temperatureF, float pressureInHg,int imuSensitivity) = 0;
  virtual void drawHitNotification( const char* targetAddress);
};
...
public:
  ...
  void clear() override
  {
    // Serial.println("Tbeam:SSD1306 clear!");
    display.clearDisplay();
  }
  void drawHitNotification(const char* targetAddress) override
  {
    // Serial.println("Tbeam:SSD1306 drawHitNotification!");
    clear();
    
    // Display "HIT" at the top center (larger text)
    display.setCursor(20, 10);
    display.setTextSize(3);
    display.setTextColor(WHITE);
    display.print("Hit!!");

    // Display "From: [address]" at the bottom center
    display.setCursor(5, 45);
    display.setTextSize(1);
    display.printf("From: %s", targetAddress);

    display.display();
  }
  void drawInterface(int batteryPercent, int signalStrength, bool hitFlag, int distance, const char* targetAddress, int windDirection, int windSpeed, float temperatureF, float pressureInHg,int imuSensitivity) override
  {
    // Serial.println("Tbeam:SSD1306 drawInterface!");
    clear();
...
public:
  ...
  void drawHitNotification( const char* targetAddress) override
  {
    ...
    // Display "HIT" at the top center (larger font)
    display.setFont(u8g2_font_logisoso24_tf);
    display.drawStr(25, 30, "HIT");
    
    // Display "From: [address]" at the bottom 
    display.setFont(u8g2_font_6x10_tr);
    display.drawStr(5, 55, ("From: " + String(targetAddress)).c_str());
    
    display.sendBuffer();
  }
  void drawInterface(int batteryPercent, int signalStrength, bool hitFlag, int distance, const char* targetAddress, int windValue, int windMode,float temperatureF, float pressureInHg,int imuSensitivity) override
  {
    clear();
