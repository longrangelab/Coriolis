...
public:
    ...
    Serial.print("QMI8658 IMU Sensitivity: ");
    Serial.println(sensitivity);

    // Nếu bạn muốn thay đổi ngưỡng AnyMotion theo độ nhạy mới:
    float newThreshold = map(sensitivity, 20, 100, 50, 500); // ví dụ, đổi ngưỡng từ 50mg → 500mg

    qmi.configAccelerometer(SensorQMI8658::ACC_RANGE_4G, SensorQMI8658::ACC_ODR_500Hz);
    qmi.configMotion(
        SensorQMI8658::ANY_MOTION_EN_X | SensorQMI8658::ANY_MOTION_EN_Y | SensorQMI8658::ANY_MOTION_EN_Z |
        SensorQMI8658::NO_MOTION_EN_X | SensorQMI8658::NO_MOTION_EN_Y | SensorQMI8658::NO_MOTION_EN_Z,
        newThreshold, newThreshold, newThreshold, 1,
        0.1, 0.1, 0.1, 1,
        1, 1);
    }
    int getSensitivity() const override {
