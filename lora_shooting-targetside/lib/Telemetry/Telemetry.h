...
public:
  ...
  virtual bool sendEnvironment(int windSpeed, bool hitFlag, int windDirection, double latitude, double longitude,int IMUsensitivity, const int &targetAddress) = 0; // Send environment data
  virtual void parseMessage(const String &message, String &senderAddress, String &messageType,String &payload) = 0; // Parse a message
  virtual void parseEnvironmentPayload(const String &payload,int &windSpeed, int &windMode, int &windDirection, double &latitude, double &longitude, int &IMUsensitivity) = 0; // Parse environment payload
  virtual void getSignalStrength(float &rssi, float &snr) =0; // Get signal strength
  virtual bool ack_hit_send(const String &name) = 0;
};
...
private:
  ...
  // static constexpr float frequencyList[10] = {
  // 433.1F, 433.35F, 433.6F, 433.85F, 434.1F,
...
public:
  ...
  bool ack_hit_send(const String &name) override {
    String msg;
    if(name == "target")
    {
      Serial.println("Waiting for receive ACK");
      if(radio.receive(msg) == RADIOLIB_ERR_NONE && msg == "ACK|")
      {
  ...
  bool sendEnvironment(int windSpeed, bool hitFlag,int windDirection, double latitude, double longitude, int IMUsensitivity, const int &targetAddress) override {
    ...
    String message = "ENVIRONMENT|" + deviceAddress + "|" + String(targetAddress) + "|" +
                     String(windSpeed) + "|"+ String(hitFlag) + "|" + String(windDirection) + "|" + String(latitude, 6) + "|" + String(longitude, 6) + "|" + String(IMUsensitivity);
    return sendMessage(message);
  }
  ...
  void parseMessage(const String &message, String &senderAddress, String &messageType,String &payload) override {
   ...
   void parseEnvironmentPayload(const String &payload,int &windSpeed, int &windMode,int &windDirection, double &latitude, double &longitude, int &IMUsensitivity) override {
    ...
    windMode       = payload.substring(p[1] + 1, p[2]).toInt();         // 0
